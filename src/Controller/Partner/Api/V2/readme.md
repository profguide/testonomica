...
А можно было увеличивать счётчик выданных доступов не в момент регистрации пользователя, а в момет привязки пользователя
к результату. И даже больше - это можно было сделать одной операцией. Провайдер передаёт пользователя и ключ результата.
Мы регистрируем пользователя (или находим), сверяемся с политикой доступов, увеличиваем счётчик выданных доступов,
...
4. **Сохранение прогресса / Привязка результата к пользователю**
   Оба варианты имеют плюсы и недостатки. Нужно выбирать по мере необходимости.

   А) **Сохраняется прогресс и выдаётся ID результата**
   Когда вопросник пройден, провайдер самостоятельно сохраняет прогресс (ответы), используя USER_KEY и получает
   RESULT_KEY. Здесь мы сверяемся с другой политикой - "Политикой Тестов". То есть проверяем прогресс каких тестов этот
   ...
   **Плюсы:** Экономия памяти Тестодрома: выше вероятность того, что провайдер сохранит прогресс только если
   пользователь совершил оплату.
   **Минусы:** Провайдер получает большой стек ответов (даже если Base64) - длинная строка, которую придётся учитывать -
   не пользоваться GET из-за ограничения длины URL, в БД использовать тип text.

   Б) **Привязка результата к пользователю**
   Когда вопросник пройден, frontend (виджет) сам сохраняет результат как гостевой на Тестодроме и возвращает
   RESULT_KEY. Провайдер передаёт RESULT_KEY и USER_KEY в специальный метод для их связки. Невозможно получить доступ к
   результату, не связанного с пользователем.
   **Плюсы:** Провайдер имеет дело с короткой строкой (ключом), которую легко передавать и хранить.
   **Минусы:** Тестодром вынужден всегда сохранять результаты, даже если оплаты (если постоплата) юзер не делать не
   собирается.

5. **Запрашивается результат по ID**
...
## Привязка пользователя к гостувому результату
